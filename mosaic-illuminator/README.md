...
'''
Select_Illumination(CurIllum = GFP Mosaic)
N = N+1
IfThenElseEntry
IfThenElseEntry
```
...
N = N+1
IfThenElseEntry
Acquire(6 14 16 1 -1 -1 10 %CurIllum% )
IfThenElseEntry
```
...
'''
Load_Regions(C:\MMpp\mmproc\journals\Mosaic As Illuminatorleach.rgn,
...
'''
Load_Regions(C:\MMpp\mmproc\journals\Mosaic As Illuminator\image.rgn,
             5 9 0 1 -1 -1 8 Untitled )
Targeted_Illumination_Update_Mask()
```
...
'''
Save_Regions(C:\MMpp\mmproc\journals\Mosaic As Illuminator\image.rgn,
             5 9 0 1 -1 -1 7 nD Snap )
```
...
n = 0
CurIllum = Device.Illumination.Setting
Clear_All_Regions(5 9 0 1 -1 -1 8 Untitled )
RunJournalEntry
Show_Message_and_Wait(Mosaic Targeted Illumination,
...
Clear_All_Regions(5 9 0 1 -1 -1 8 Untitled )
RunJournalEntry
Enter_Variable(pulseTimePoint,
               ,
               Mosaic Targeted Illumination,
               ,
               ,
               ,
               ,
               Before which time point are we pulsing?,
               %GRAYS%
%COLOR%
%BOTH%,
               ,
...
Enter_Variable(pulseDuration,
               ,
               Moasaic Targeted Illumination,
               ,
               ,
               ,
               ,
               What is the duration of the laser pulse (in ms)?,
               %GRAYS%
%COLOR%
%BOTH%,
               ,
